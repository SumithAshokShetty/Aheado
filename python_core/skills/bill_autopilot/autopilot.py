...
class BillAutopilotSkill:
    ...
    DEFAULT_SPEND_CAP = 50.00  # Default AP2 Threshold in USD
    ...
    def execute(cls, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Blocks and prompts HITL confirmation if spend bounds are exceeded.
        
        Args:
            arguments (dict): Expected fields:
                - bill_title (str): Name of bill provider, e.g., "PGE Energy Corp".
                - amount (float): Total bill amount due.
                - due_date (str): Due date of the statement.
                - user_spend_cap (float, optional): Custom AP2 boundary.
        ...
        amount = float(arguments.get("amount", 29.99))
        due_date = arguments.get("due_date", "Soon")
        spend_cap = float(arguments.get("user_spend_cap", cls.DEFAULT_SPEND_CAP))
